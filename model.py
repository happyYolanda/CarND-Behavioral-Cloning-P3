...
def cropness(image,steering=0.0,tx_lower=-20,tx_upper=20,ty_lower=-2,ty_upper=2,rand=True):
    ...
    cropness = image[horizon+ty:bonnet+ty,col_start+tx:col_end+tx,:]
    image = cv2.resize(cropness,(64,64),cv2.INTER_AREA)
...
def generate_training_set(X_train,X_left,X_right,Y_train):
    ...
    image,steering = shearness(image,steering,shear_range=100) 
    image,steering = cropness(image,steering,tx_lower=-20,tx_upper=20,ty_lower=-10,ty_upper=10)
    image,steering = flipness(image,steering)
...
def generate_validation_set(X_val,Y_val):
    ...
    for i in range(len(X_val)):
        ...
        X[i],Y[i] = cropness(x,y,tx_lower=0,tx_upper=0,ty_lower=0,ty_upper=0)
    return X,Y
...
def generate_train_batch(X_train,X_left,X_right,Y_train,batch_size = 32):
    ...
    while 1:
        for i_batch in range(batch_size):
            x,y = generate_training_set(X_train,X_left,X_right,Y_train)
...
history = model.fit_generator(train_generator,
                    samples_per_epoch=2000, nb_epoch=nb_epoch,
